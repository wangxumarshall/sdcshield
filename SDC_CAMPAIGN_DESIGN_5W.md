...
**自愈机制**：`.done` 阶段标记断点续跑；看门狗每 2h 检测进程死亡自动重启；磁盘守卫 <5G 中止；外层 timeout 按测试数计算（`-t` 每测试语义）。
...
5. **mesh_upi_sse_asymm_distrib_int 失败已定性为测试协议竞态假阳性**（算术级铁证：expected_sum == 部分重填块之和；40/40 失败满足竞态签名）——修复后 M7 重跑才是该族的干净检验；**真 SDC 若存在曾被此噪声掩盖**

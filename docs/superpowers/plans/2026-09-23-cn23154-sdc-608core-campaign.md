...
- [x] **Step 1: 缩减维度逐个实验（每步只改一个因素，A/B 验证：原版仍复现 + 缩减版失败率对比而非是否曾失败）**：顺序 = 1) 去 Si.ion（单段 SCF）；2) MAXIT_SCF 下调到故障迭代+余量；3) FD 网格缩小（Si.inpt 拷贝改 FD_GRID，记录每档）；4) NSTATES/XLSDFT_NSTATES 缩减；5) NT 重编译递减（36->18->9->4->2->1，全参数留痕、原 a.out 不动）；6) NCOMMS 降 rank 数（若源码支持）。
- [x] **Step 2: 每版本在 F 核重复 >=5 次、在 C1 >=5 次，失败率表格 + 版本目录（源/构建/输入/日志）**
- [x] **Step 3: 定稿 minimal-reproducer/：README（构建+运行+判定+预期现象+统计复现率）、自包含输入、校验脚本、绑定命令；由另一研究者可从零复现的自检清单**
- [x] **Step 4: N/A — Branch A 已确认故障核 cpu139（P4 15/15），无 Branch B 情形；原文字: 若全程无确认故障核（Branch B 延续）：不编造最小用例，报告"未获得稳定触发条件"+ 已达证据强度（spec 强制）**
...
- **v6c 收官 — 空间 x 减元素数中性（2026-09-24 20:05）**：job 1721020（19:07-19:58, MIN_V6C_JOB_DONE）13 行：F 侧 **7/7 崩**（canary 28s + F_1-F_5 86-167s + F 锚 41s，全 rank3 [1,3]，崩溃线程 psr 7/7 全 cpu139 尾采样，全 SEGV_MAPERR；**canonical 0x3ffX 栈带 4 例全部 @stencil a.out+0x258f00 = 同作业最强崩溃点集中** [F_1 0x3ffa5726e000 / F_2 0x3ff971ebac00 / F_3 0x3ffd6b563340 / F_4 0x3ffd12e56b80]；byte-6 heap 2 [canary 0x31 / F 锚 0x33, 双双 libomp 顶层簇] + byte-6 @0x4000 区 1 [F_5 0x42 @swap37 簇 a.out+0x139ac4]）；C1 侧 **6/6 净**（v2 锚 813s scf 1.822e-01/1.518e-01/1.856e-02 跨作业一致 + C1_1-5 287-290s rc=0，scf iter3 2.945e-01 五 rep 一致，E3 -1680.94758271~-75 展幅 4.2e-8，replica 0）。**canonical 栈带再解释（推断+实证）**：dattach /proc/self/maps 主栈 0xffffc1185000 / libomp-libc-ld 0x4000_2d00-3f00 → 0x3ff9-3ffd 带 = libs 下方深 mmap 区 = worker pthread 栈落点，"野指针"再解读为栈带指针损坏。几何澄清：v2 源 CELL 非立方（102.87105848×82.296846784×82.296846784），v6c CELL x 102.87105848→82.296846784 保持元素 10.287105848³ 与边界线逐位不变（丢 254 原子恰为 ni 8-9 元素，kept-maxX 82.28952 间隙干净）。频率全作业窗 cpu139 210/210 @1999 / cpu140 210/210 @1998；新红线 cpu436 整窗 1458（19:08-19:57 负载相关，全天其余 2805 采样正常，槽位核非归因）。**判定：NELEMS 三维缩减（z/y/x）F 侧累计 21/21 全崩全 cpu139 → SDC 与空间区域无关；转 v6d 终点 2 2 4**（job 1721399 20:01:35 起，16 元素 1/rank 真·最小形态 / 125 原子 / CELL 20.574211696²×41.148423392 / FD_GRID 32 30 60，工件 versions/v6d_nelems224/ + 脚本 scripts/job_min_v6d_nelems224.sh；提交后首 3 运行 canary 67s / F 锚 187s / F_1 83s 已全崩，全 rank3 全 psr=cpu139 全 canonical 0x3ffX @stencil）。
- **v6d 终点收官 — NELEMS 轴关闭（2026-09-24 20:50）**：job 1721399（20:01-20:47, MIN_V6D_JOB_DONE）13 行：F 侧 **7/7 崩**（canary 67s + F_1-F_5 74-151s + F 锚 187s，全 rank3 [1,3]，崩溃线程 psr 7/7 全 cpu139，全 SEGV_MAPERR；**canonical 0x3ffX 栈带 5/7 全部 @stencil a.out+0x258f00 = 集中达峰** [canary 0x3ffd6c507780 / F 锚 0x3ff98191e000 / F_1 0x3ff9f18e8c80 / F_2 0x3ffe6177b640 / F_5 0x3ffb95445400, 0x3ffe 首次入带]；byte-6 heap 1 [F_3 0x63 @libomp 顶层簇] + byte-6 @0x4000 区 1 [F_4 0x7c @swap37 簇]）；C1 侧 **6/6 净**（v2 锚 813s scf 1.822e-01/1.518e-01/1.856e-02 三作业连续一致 + C1_1-5 196-200s rc=0，scf 7.796e-01/5.784e-01/3.103e-01 五 rep 一致，E3 -206.7261637648xx 展幅 2.2e-14，replica 0）。几何：NELEMS 2 2 4 @ NCOMMS 2 2 4 = 16 元素 1/rank 真·最小形态，CELL 20.574211696²×41.148423392，FD_GRID 32 30 60，125 原子，rank3 = 恰 1 元素（ni 1/nj 1/nk 0）。频率全作业窗 cpu139 191/191 @1999 / cpu140 191/191 @1998；新红线 cpu558 整窗 1391（20:01:37 起 = job 启动，负载相关，NUMA14，非归因）。**判定：NELEMS 轴四级全中性关闭（v6a z 320 / v6b y 160 / v6c x 128 / v6d 16），F 侧累计 28/28 全崩全 cpu139，元素 640→16（1/40）原子 5120→125 — SDC 与问题规模完全解耦；转 Task 10 Step 3 最小复现体定稿**（以 versions/v6d_nelems224/ 为基底：README + 自含输入 + 验证脚本 + 绑定说明 + 复现率 [F 7/7 = 100%, 中位崩溃时延 ~94s]）。
- **Step 3 完成（2026-09-24 21:20，job 1721859 端到端自证）**：$RES/minimal-reproducer/ 定稿 — 自含包（a.out NT9 763c6843f504e1f7 + libomp.so + v6d 输入三件 + rf_nc16_F/C1 + reproduce.sh/verify.sh/job_repro.sh/freqmon.sh + README）。端到端自证（仅包内文件）：F×3 3/3 REPRO_CRASH_ATTRIBUTED（rank3/psr139/cpu139 1999 VALID；79.6s canonical_3ffc @stencil+0x258f00、150.9s byte6-lib_b2 @swap37+0x139ac4、74.4s canonical_3ffa @stencil+0x258f00）+ C1×1 CLEAN_CONTROL_PASS（196.6s rc=0，scf 三迭代金值，E3=-206.726163764884，cpu140 1999 VALID）；零红线核；判定表 runs/verify_table.txt。签名一致性核对通过（地址类/崩溃点/归因不变量/时延四项全同 v6d 家族）。复现率定稿：本包形态 F 10/10、C1 7/7；阶梯累计 F 28/28。开发留痕：attempt-1 job 1721850 全 rc=127（reproduce.sh 漏 set --，setvars.sh 吞位置参数 → mpirun not found，已修复，attempt 日志保留）；verify.sh scf 金值 grep 被 --tag-output 行中标签切断，改 detag 后匹配重判。12:33 旧 V2 期骨架被替换。**Task 10 全部完成，转 Task 11 最终交付。**

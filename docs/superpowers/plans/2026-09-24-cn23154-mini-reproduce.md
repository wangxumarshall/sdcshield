...
> 完成流程补充 (2026-09-25 08:50 核实脚本后定): (a) 跨天普查 — 作业末尾自动 analyze 只吃 `ls -t freq-*.csv | head -1` (最新一天), 全扫跨 ~2 天时 summary.txt 末尾的红线普查不完整; 完成时手动重跑: `cat $NEW/environment/freq-2026092[5-9].csv > $NEW/environment/freq-allsweep.csv && bash $NEW/analyze_probes.sh $NEW/environment/freq-allsweep.csv` (per-probe 判定自含于各 run 的 freq_probe.tsv 5s 定向采样, 不受影响; census 窗口 = 全扫日起全部采样, 含同日早间消融作业, 超集口径, 报告中如实标注)。(b) 节奏实测: P1-P4 含开销 ~240s/核 → 608 核 ≈ 40h ≈ 3.4 个 -T 43200 段; 每 12h 段被杀后 djob 消失 + rows<608 → cron 重投同一脚本直至 608 行。(c) rows/P<C>.tsv 字段 $6=rc、$NF=rundir, 与 analyze 取列已核对一致。
> 频率实况观察 (2026-09-25 08:47): 全扫首 23 分钟 3 例空闲停摆 <1500MHz — cpu308/327 (1486/1479, 08:06:50-08:08:10, 消融窗口, 已回升) + cpu338 (1473, 08:24:26 即全扫启动+3s 起, 截至 08:45:56 仍停摆; 同期其余 607 核含 cpu139=1998 全部 >=1996)。协议处理: 记录在案不深究; 各核判定只看其探针窗口 freq_probe.tsv (探针负载上来后预期回 2.0GHz; 若窗口内仍 <1500 → PROBE_EXCLUDED_FREQ 如实排除); 终稿普查节列出全部 <1500 核并解释 "窗口外空闲停摆 ≠ 判定排除, EXCLUDED_FREQ 清单才是判定口径"。**数据源铁律 (本日踩坑实证): 登录节点 sysfs 没有计算节点的这些 CPU, 任何 "当前频率" 必须来自 freqmon CSV (10s) 或作业内读取**; README §9 收官时补为第 7 坑。
> 段史补记 (2026-09-25 21:55): 第一段 job 1724877 (-T 43200, 08:24:23→20:24:31 到点被杀): 有效入库 176 行 = P0-P175 (本段新跑 172 核 + pilot2 已入账 P0/P114/P131/P139); P139 rc=139 为唯一崩溃 (92.5s, 复用 pilot2 行)。P176/P177 为被杀伪影 (rc=1: 215.3s 截断 / 1.8s 启动即杀), rows 标记+残缺目录已归档 probes2/killed_1724877/ 并删标记待重跑。作业空窗期脚本修订 (未触碰运行中脚本): ① -T 43200→151200 (dqueue 实查 jobWallClockLimit=UNLIMITED, 单段覆盖剩余 432 核, 修正上注 "~3.4 段" 预估); ② TMO 360→480 (实测 n=176: p50=242.8s / p90=298.0s / max=310.2s, 360 仅 50s 裕量); ③ 头注节奏注释同步 (243-310s/core 实测替换 ~205s 旧估)。第二段 job 1728606 (2026-09-25 21:44:54 起, -T 151200): 首核 P176 已以 tmo=480 重跑 (21:46:07 start), .out 确认追加模式 (旧段证据完好); 剩余 432 核预计 30-37h, 完成时作业末尾自动 analyze (最新一天 CSV), 随后手动跨天普查 (freq-allsweep.csv) 为准。

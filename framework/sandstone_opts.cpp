...
Common command-line options are:
 ...
 -T <time>, --total-time=<time>
     Specify the minimum run time for the program. A special value for <time> of
     "forever" causes the program to loop indefinitely. The default units for
     <time> are milliseconds, with s, m, and h available for seconds, minutes or
     hours.
 ...
 -t <test-time>
     Specify the execution time per test for the program in ms. Value for this
     field can also be specified with a label s, m, h for seconds, minutes or
     hours. Example: 200ms, 2s or 2m.
 ...
 --max-test-loop-count <NUMBER>
     Specifies how many times each test executes its main loop. This option
     augments the time-based options in that the test will end if either the
 ...
 --max-messages <NUMBER>
     ...
     thread per test invocation. A value of less than or equal to 0 means that
     there is no limit. The default value is 5.
 --max-logdata <NUMBER>
     Limits the maximum number of bytes of binary data that can be logged by a
     single thread per test invocation. A value of less than or equal to 0 means
     that there is no limit. The default value is 128. Sandstone will not log
     partial data, so if the binary data would cause the thread to exceed this
     threshold it simply will not be output.
 -n <NUMBER>, --threads=<NUMBER>
     Set the number of threads to be run to <NUMBER>. If not specified or if 0
     is passed, then the test defaults to the number of CPUs in the system.
 ...
 -o, --output-log <FILE>
     ...
     auto-generated by the program. Use -o /dev/null to suppress creation of any
     file.
 ...
 --1sec, --30sec, --2min, --5min
     Run for the specified amount of time in the option. In this mode, the
     program prioritizes test execution based on prior detections. These
     options are intended to drive coverage over multiple runs. Test priority
     is ignored when running in combination with the --test-list-file option.
 --test-list-file <file path>
     Specifies the tests to run in a text file. This will run the tests in the
     order they appear in the file and also allows you to vary the individual
     test durations. See the User Guide for details.
 --test-list-randomize
    ...
    int parse_args(SandstoneApplicationConfig* app_cfg, ProgramOptions& opts, char** argv)
    {
        ...
        auto assign_maybe_split = [&](auto&& vec) {
            ...
            for (const char* elem : vec) {
                if (std::string_view{elem}.find(',') == std::string_view::npos) { // is single name

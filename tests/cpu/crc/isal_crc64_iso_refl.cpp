...
static int isal_crc64_iso_refl_init(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}
...
static int isal_crc64_iso_refl_run(struct test *test, int cpu) {
    ...
    do {
        ...
        if (!passed) {
            report_fail_msg("isal_crc64_iso_refl: CRC mismatch or consistency failure");
            return EXIT_FAILURE;
        }
...
static int isal_crc64_iso_refl_finish(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

DECLARE_TEST(isal_crc64_iso_refl, "ISA-L CRC64 ISO 3309 reflected (random data)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = isal_crc64_iso_refl_init,
    .test_run = isal_crc64_iso_refl_run,
    .test_cleanup = isal_crc64_iso_refl_finish,

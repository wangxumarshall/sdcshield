...
static int fma_tail_avx512_init(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

#ifdef __aarch64__
static int fma_tail_avx512_run(struct test *test, int cpu) {
    (void)cpu;
    /* randomization hardening H9'/P12: framework RNG (per-thread stream,
     * -s reproducible) replaces std::mt19937; range [-10.0, 10.0) mapped
     * from frandom's [0,1). */
    auto dist = []() { return frandom_scale((double)(10.0) - (double)(-10.0)) + (double)(-10.0); };
    ...
    do {
        ...
        if (!passed) {
            ...
            // ---- 输出日志（与 x86 版本完全一致） ----
            fprintf(stderr, "fma_tail_avx512: Iter %lu, a[0..3]=%.12e %.12e %.12e %.12e\n",
            ...
            report_fail_msg("fma_tail_avx512: FMA tail precision mismatch or consistency failure");
            return EXIT_FAILURE;
        }
...
static int fma_tail_avx512_finish(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

DECLARE_TEST(fma_tail_avx512, "AVX-512 double FMA tail precision test (NEON version)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = fma_tail_avx512_init,
    .test_run = fma_tail_avx512_run,
    .test_cleanup = fma_tail_avx512_finish,

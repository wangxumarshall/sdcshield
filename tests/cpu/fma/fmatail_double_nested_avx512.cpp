...
static int fmatail_double_nested_avx512_init(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

#ifdef __aarch64__
static int fmatail_double_nested_avx512_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        for (int group = 0; group < INNER_ITERATIONS; ++group) {
            ...
            fprintf(stderr, "fmatail_double_nested_avx512: Iter %lu, Group %d, a[0..3]=%.12e %.12e %.12e %.12e\n",
        ...
        if (!all_passed) {
            report_fail_msg("fmatail_double_nested_avx512: Some group failed");
            return EXIT_FAILURE;
        }
...
static int fmatail_double_nested_avx512_finish(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

DECLARE_TEST(fmatail_double_nested_avx512, "AVX-512 double FMA exhaustive tail precision with double nested loops (NEON version)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = fmatail_double_nested_avx512_init,
    .test_run = fmatail_double_nested_avx512_run,
    .test_cleanup = fmatail_double_nested_avx512_finish,

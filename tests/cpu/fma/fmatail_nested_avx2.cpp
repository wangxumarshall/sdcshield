...
static int fmatail_nested_avx2_init(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

#ifdef __aarch64__
static int fmatail_nested_avx2_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        for (int group = 0; group < INNER_ITERATIONS; ++group) {
            ...
            fprintf(stderr, "fmatail_nested_avx2: Iter %lu, Group %d, a[0..3]=%.12e %.12e %.12e %.12e\n",
        ...
        if (!all_passed) {
            report_fail_msg("fmatail_nested_avx2: Some group failed");
            return EXIT_FAILURE;
        }
...
static int fmatail_nested_avx2_finish(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

DECLARE_TEST(fmatail_nested_avx2, "AVX2 double FMA exhaustive tail precision with nested loops (NEON version)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = fmatail_nested_avx2_init,
    .test_run = fmatail_nested_avx2_run,
    .test_cleanup = fmatail_nested_avx2_finish,

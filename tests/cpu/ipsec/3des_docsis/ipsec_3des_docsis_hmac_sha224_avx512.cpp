 ...
 * "-O ipsec_3des_docsis_hmac_sha224_avx512.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int hmac_sha224_avx512_run(struct test *test, int cpu) {
    ...
    struct hmac_sha224_avx512_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        hmac_sha224_avx512_compute_golden(&local_d);
        ...
        memcmp_or_fail(mac, local_d.golden_mac, SHA224_DIGEST_SIZE, "HMAC-SHA224 digest mismatch (AVX-512)");
    }
...
static int hmac_sha224_avx512_cleanup(struct test *test) {
    struct hmac_sha224_avx512_data *d = (hmac_sha224_avx512_data *)test->data;
    free(d->plaintext);

 ...
 *
 * @test @b ipsec_3des_docsis_hmac_sha256_avx512
 * @parblock
 * This test encrypts random data using 3DES-DOCSIS encryption and computes
 * an HMAC-SHA256 message authentication code on the ciphertext via OpenSSL
 ...
 * "-O ipsec_3des_docsis_hmac_sha256_avx512.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
struct hmac_sha256_avx512_data {
    uint8_t des3_key[DES3_KEY_SIZE];
    uint8_t des3_iv[DES3_IV_SIZE];
    uint8_t hmac_key[SHA256_KEY_SIZE];
    ...
    uint8_t golden_mac[SHA256_DIGEST_SIZE];
};

static void hmac_sha256(const uint8_t *key, const uint8_t *msg, size_t len, uint8_t *out) {
    HMAC_CTX *ctx = s_HMAC_CTX_new();
    s_HMAC_Init_ex(ctx, key, SHA256_KEY_SIZE, s_EVP_sha256(), NULL);
...
static void hmac_sha256_avx512_compute_golden(struct hmac_sha256_avx512_data *d) {
    des3_encrypt(d->des3_key, d->des3_iv, d->plaintext, d->golden_ciphertext, d->datasize);
    hmac_sha256(d->hmac_key, d->golden_ciphertext, d->datasize, d->golden_mac);
}

static int hmac_sha256_avx512_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_des_ede3_cbc && s_HMAC_CTX_new && s_EVP_sha256) {
        struct hmac_sha256_avx512_data *d = (struct hmac_sha256_avx512_data *)malloc(sizeof(*d));
        ...
        memset_random(d->hmac_key, SHA256_KEY_SIZE);
        memset_random(d->plaintext, d->datasize);

        hmac_sha256_avx512_compute_golden(d);
...
static int hmac_sha256_avx512_run(struct test *test, int cpu) {
    struct hmac_sha256_avx512_data *d = (struct hmac_sha256_avx512_data *)test->data;

    uint8_t *ciphertext = (uint8_t *)malloc(d->datasize);
    uint8_t *decrypted = (uint8_t *)malloc(d->datasize);
    uint8_t *mac = (uint8_t *)malloc(SHA256_DIGEST_SIZE);
    ...
    struct hmac_sha256_avx512_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        hmac_sha256_avx512_compute_golden(&local_d);
        ...
        hmac_sha256(local_d.hmac_key, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, SHA256_DIGEST_SIZE, "HMAC-SHA256 digest mismatch (AVX-512)");
    }
...
static int hmac_sha256_avx512_cleanup(struct test *test) {
    struct hmac_sha256_avx512_data *d = (hmac_sha256_avx512_data *)test->data;
    free(d->plaintext);
...
DECLARE_TEST(ipsec_3des_docsis_hmac_sha256_avx512, "Intel IPSEC 3DES-DOCSIS w/ HMAC-SHA256 AVX512")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = hmac_sha256_avx512_init,
    .test_run = hmac_sha256_avx512_run,
    .test_cleanup = hmac_sha256_avx512_cleanup,

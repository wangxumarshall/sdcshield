 ...
 *
 * @test @b ipsec_3des_docsis_hmac_sha512_avx512
 * @parblock
 ...
 * "-O ipsec_3des_docsis_hmac_sha512_avx512.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
struct hmac_sha512_avx512_data {
...
static void hmac_sha512_avx512_compute_golden(struct hmac_sha512_avx512_data *d) {
    des3_encrypt(d->des3_key, d->des3_iv, d->plaintext, d->golden_ciphertext, d->datasize);
    hmac_sha512(d->hmac_key, d->golden_ciphertext, d->datasize, d->golden_mac);
}

static int hmac_sha512_avx512_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_des_ede3_cbc && s_HMAC_CTX_new && s_EVP_sha512) {
        struct hmac_sha512_avx512_data *d = (struct hmac_sha512_avx512_data *)malloc(sizeof(*d));
        ...
        hmac_sha512_avx512_compute_golden(d);
...
static int hmac_sha512_avx512_run(struct test *test, int cpu) {
    struct hmac_sha512_avx512_data *d = (struct hmac_sha512_avx512_data *)test->data;
    ...
    struct hmac_sha512_avx512_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        hmac_sha512_avx512_compute_golden(&local_d);
        ...
        hmac_sha512(local_d.hmac_key, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, SHA512_DIGEST_SIZE, "HMAC-SHA512 digest mismatch (AVX-512)");
    }
...
static int hmac_sha512_avx512_cleanup(struct test *test) {
    struct hmac_sha512_avx512_data *d = (hmac_sha512_avx512_data *)test->data;
    free(d->plaintext);
...
DECLARE_TEST(ipsec_3des_docsis_hmac_sha512_avx512, "Intel IPSEC 3DES-DOCSIS w/ HMAC-SHA512 AVX512")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = hmac_sha512_avx512_init,
    .test_run = hmac_sha512_avx512_run,
    .test_cleanup = hmac_sha512_avx512_cleanup,

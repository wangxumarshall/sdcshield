 ...
 *
 * @test @b ipsec_aes128_cbc_aes_cmac_sse
 * @parblock
 ...
 * "-O ipsec_aes128_cbc_aes_cmac_sse.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static void aes_cmac_sse_compute_golden(struct aes_cmac_sse_data *d) {
...
static int aes_cmac_sse_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_aes_128_cbc() && s_CMAC_CTX_new) {
        struct aes_cmac_sse_data *d = (struct aes_cmac_sse_data *)malloc(sizeof(*d));
...
static int aes_cmac_sse_run(struct test *test, int cpu) {
    struct aes_cmac_sse_data *d = (struct aes_cmac_sse_data *)test->data;
    ...
    struct aes_cmac_sse_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        aes_cmac_sse_compute_golden(&local_d);
        aes_encrypt(local_d.aes_key, local_d.aes_iv, local_d.plaintext, ciphertext, local_d.datasize);
        memcmp_or_fail(ciphertext, local_d.golden_ciphertext, local_d.datasize, "AES-128-CBC ciphertext mismatch (SSE)");

        aes_decrypt(local_d.aes_key, local_d.aes_iv, ciphertext, decrypted, local_d.datasize);
        memcmp_or_fail(decrypted, local_d.plaintext, local_d.datasize, "AES-128-CBC decryption mismatch (SSE)");

        aes_cmac_96(local_d.cmac_key, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, CMAC_96_DIGEST_SIZE, "AES-CMAC-96 digest mismatch (SSE)");
    }
...
static int aes_cmac_sse_cleanup(struct test *test) {
    struct aes_cmac_sse_data *d = (aes_cmac_sse_data *)test->data;
    free(d->plaintext);
...
DECLARE_TEST(ipsec_aes128_cbc_aes_cmac_sse, "Intel IPSEC AES128-CBC w/ AES-CMAC-96 SSE")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = aes_cmac_sse_init,
    .test_run = aes_cmac_sse_run,
    .test_cleanup = aes_cmac_sse_cleanup,

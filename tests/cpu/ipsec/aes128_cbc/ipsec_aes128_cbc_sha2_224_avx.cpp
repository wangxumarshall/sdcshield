 ...
 *
 * @test @b ipsec_aes128_cbc_sha2_224_avx
 * @parblock
 ...
 * "-O ipsec_aes128_cbc_sha2_224_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static void sha2_224_avx_compute_golden(struct sha2_224_avx_data *d) {
...
static int sha2_224_avx_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_aes_128_cbc() && s_HMAC_CTX_new && s_EVP_sha224) {
        struct sha2_224_avx_data *d = (struct sha2_224_avx_data *)malloc(sizeof(*d));
...
static int sha2_224_avx_run(struct test *test, int cpu) {
    struct sha2_224_avx_data *d = (struct sha2_224_avx_data *)test->data;
    ...
    struct sha2_224_avx_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        sha2_224_avx_compute_golden(&local_d);
        ...
        hmac_sha224(local_d.hmac_key, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, SHA224_DIGEST_SIZE, "HMAC-SHA224 digest mismatch (AVX)");
    }
...
static int sha2_224_avx_cleanup(struct test *test) {
    struct sha2_224_avx_data *d = (sha2_224_avx_data *)test->data;
    free(d->plaintext);
...
DECLARE_TEST(ipsec_aes128_cbc_sha2_224_avx, "Intel IPSEC AES128-CBC SHA2_224 AVX")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = sha2_224_avx_init,
    .test_run = sha2_224_avx_run,
    .test_cleanup = sha2_224_avx_cleanup,

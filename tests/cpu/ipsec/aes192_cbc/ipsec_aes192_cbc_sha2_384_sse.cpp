 ...
 *
 * @test @b ipsec_aes192_cbc_sha2_384_sse
 * @parblock
 ...
 * "-O ipsec_aes192_cbc_sha2_384_sse.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int aes_hmac_sse_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_DIGEST_SIZE, "HMAC-SHA-384 digest mismatch (SSE)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes192_cbc_sha2_384_sse, "Intel IPSEC AES192-CBC SHA2_384 SSE")

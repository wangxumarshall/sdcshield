 ...
 *
 * @test @b ipsec_aes192_cbc_sha2_512_sse
 * @parblock
 ...
 * "-O ipsec_aes192_cbc_sha2_512_sse.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int aes_hmac_sse_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        hmac_sha512(local_d.hmac_key, HMAC_KEY_SIZE, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_DIGEST_SIZE, "HMAC-SHA-512 digest mismatch (SSE)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes192_cbc_sha2_512_sse, "Intel IPSEC AES192-CBC SHA2_512 SSE")

 ...
 *
 * @test @b ipsec_aes192_ctr_hmac_sha1_avx
 * @parblock
 ...
 * "-O ipsec_aes192_ctr_hmac_sha1_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int aes_hmac_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(ciphertext, local_d.golden_ciphertext, local_d.datasize, "AES-192-CTR ciphertext mismatch (AVX)");

        aes_ctr_decrypt(local_d.aes_key, local_d.aes_iv, ciphertext, decrypted, local_d.datasize);
        memcmp_or_fail(decrypted, local_d.plaintext, local_d.datasize, "AES-192-CTR decryption mismatch (AVX)");
...
DECLARE_TEST(ipsec_aes192_ctr_hmac_sha1_avx, "Intel IPSEC AES192-CTR w/ HMAC_SHA1-96 AVX")

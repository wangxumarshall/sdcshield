 ...
 *
 * @test @b ipsec_aes192_ctr_hmac_sha224_avx
 * @parblock
 ...
 * "-O ipsec_aes192_ctr_hmac_sha224_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
#define DATA_SIZE_DEFAULT (1024u)
#define AES_KEY_SIZE (24)
#define AES_IV_SIZE (16)
#define HMAC_KEY_SIZE (28) // SHA-224 key size can be 28 bytes or larger
...
static void aes_hmac_avx_compute_golden(struct aes_hmac_avx_data *d) {
    ...
    hmac_sha224(d->hmac_key, HMAC_KEY_SIZE, d->golden_ciphertext, d->datasize, d->golden_mac);
}
...
static int aes_hmac_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        hmac_sha224(local_d.hmac_key, HMAC_KEY_SIZE, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_DIGEST_SIZE, "HMAC-SHA-224 digest mismatch (AVX)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes192_ctr_hmac_sha224_avx, "Intel IPSEC AES192-CTR w/ HMAC SHA-224 AVX")

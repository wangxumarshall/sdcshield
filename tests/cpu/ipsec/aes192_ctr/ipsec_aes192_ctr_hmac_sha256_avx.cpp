 ...
 *
 * @test @b ipsec_aes192_ctr_hmac_sha256_avx
 * @parblock
 * This test encrypts random data using AES-192-CTR encryption and computes
 * an HMAC-SHA-256 message authentication code on the ciphertext via OpenSSL
 ...
 * "-O ipsec_aes192_ctr_hmac_sha256_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static void hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *msg, size_t len, uint8_t *out) {
    HMAC_CTX *ctx = s_HMAC_CTX_new();
    s_HMAC_Init_ex(ctx, key, key_len, s_EVP_sha256(), NULL);
...
static void aes_hmac_avx_compute_golden(struct aes_hmac_avx_data *d) {
    ...
    hmac_sha256(d->hmac_key, HMAC_KEY_SIZE, d->golden_ciphertext, d->datasize, d->golden_mac);
}
...
static int aes_hmac_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        hmac_sha256(local_d.hmac_key, HMAC_KEY_SIZE, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_DIGEST_SIZE, "HMAC-SHA-256 digest mismatch (AVX)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes192_ctr_hmac_sha256_avx, "Intel IPSEC AES192-CTR w/ HMAC SHA-256 AVX")

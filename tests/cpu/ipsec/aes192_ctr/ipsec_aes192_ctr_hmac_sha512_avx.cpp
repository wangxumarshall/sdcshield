 ...
 *
 * @test @b ipsec_aes192_ctr_hmac_sha512_avx
 * @parblock
 ...
 * "-O ipsec_aes192_ctr_hmac_sha512_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int aes_hmac_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_DIGEST_SIZE, "HMAC-SHA-512 digest mismatch (AVX)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes192_ctr_hmac_sha512_avx, "Intel IPSEC AES192-CTR w/ HMAC SHA-512 AVX")

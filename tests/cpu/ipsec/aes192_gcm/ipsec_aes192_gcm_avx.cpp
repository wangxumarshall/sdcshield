 ...
 *
 * @test @b ipsec_aes192_gcm_avx
 * @parblock
 * This test encrypts random data using AES-192-GCM encryption via OpenSSL
 * using the AVX optimized code path. It generates an authentication tag,
 ...
 * "-O ipsec_aes192_gcm_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
#define DATA_SIZE_DEFAULT (1024u)
#define AES_KEY_SIZE (24)
#define AES_IV_SIZE (12)
#define GCM_TAG_SIZE (16)

struct aes_gcm_avx_data {
...
static int aes_gcm_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(ciphertext, d->golden_ciphertext, d->datasize, "AES-192-GCM ciphertext mismatch (AVX)");
        memcmp_or_fail(tag, d->golden_tag, GCM_TAG_SIZE, "AES-192-GCM tag mismatch (AVX)");
        ...
        memcmp_or_fail(decrypted, d->plaintext, d->datasize, "AES-192-GCM decryption mismatch (AVX)");
    }
    free(ciphertext);
    free(decrypted);
    free(tag);
...
static int aes_gcm_avx_cleanup(struct test *test) {
    struct aes_gcm_avx_data *d = (aes_gcm_avx_data *)test->data;
    free(d->plaintext);
...
DECLARE_TEST(ipsec_aes192_gcm_avx, "Intel IPSEC AES192-GCM AVX")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = aes_gcm_avx_init,
    .test_run = aes_gcm_avx_run,
    .test_cleanup = aes_gcm_avx_cleanup,

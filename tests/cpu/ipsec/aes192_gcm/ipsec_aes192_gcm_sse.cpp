 ...
 *
 * @test @b ipsec_aes192_gcm_sse
 * @parblock
 * This test encrypts random data using AES-192-GCM encryption via OpenSSL
 * using the SSE optimized code path. It generates an authentication tag,
 ...
 * "-O ipsec_aes192_gcm_sse.datasize=N" (1024..64MB, multiple of 16, default 1024). */
#define DATA_SIZE_DEFAULT (1024u)
#define AES_KEY_SIZE (24)
#define AES_IV_SIZE (12)
#define GCM_TAG_SIZE (16)

struct aes_gcm_sse_data {
...
static int aes_gcm_sse_run(struct test *test, int cpu) {
    struct aes_gcm_sse_data *d = (struct aes_gcm_sse_data *)test->data;
    ...
    struct aes_gcm_sse_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        aes_gcm_sse_compute_golden(&local_d);
        aes_gcm_encrypt(local_d.aes_key, local_d.aes_iv, local_d.plaintext, ciphertext, tag, local_d.datasize);
        memcmp_or_fail(ciphertext, local_d.golden_ciphertext, local_d.datasize, "AES-192-GCM ciphertext mismatch (SSE)");
        memcmp_or_fail(tag, local_d.golden_tag, GCM_TAG_SIZE, "AES-192-GCM tag mismatch (SSE)");

        int ret = aes_gcm_decrypt(local_d.aes_key, local_d.aes_iv, ciphertext, decrypted, tag, local_d.datasize);
        if (ret != 1) {
            report_fail_msg("AES-192-GCM decryption failed (tag verification failed) (SSE)");
        }
        memcmp_or_fail(decrypted, local_d.plaintext, local_d.datasize, "AES-192-GCM decryption mismatch (SSE)");
    }
    free(ciphertext);
    free(decrypted);
    free(tag);
...
static int aes_gcm_sse_cleanup(struct test *test) {
    struct aes_gcm_sse_data *d = (struct aes_gcm_sse_data *)test->data;
    free(d->plaintext);
...
DECLARE_TEST(ipsec_aes192_gcm_sse, "Intel IPSEC AES192-GCM SSE")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = aes_gcm_sse_init,
    .test_run = aes_gcm_sse_run,
    .test_cleanup = aes_gcm_sse_cleanup,

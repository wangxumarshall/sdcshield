 ...
 *
 * @test @b ipsec_aes256_cbc_hmac_sha1_avx
 * @parblock
 ...
 * "-O ipsec_aes256_cbc_hmac_sha1_avx.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int aes_hmac_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(ciphertext, local_d.golden_ciphertext, local_d.datasize, "AES-256-CBC ciphertext mismatch (AVX)");

        aes_decrypt(local_d.aes_key, local_d.aes_iv, ciphertext, decrypted, local_d.datasize);
        memcmp_or_fail(decrypted, local_d.plaintext, local_d.datasize, "AES-256-CBC decryption mismatch (AVX)");

        hmac_sha1_96(local_d.hmac_key, HMAC_KEY_SIZE, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_SHA1_96_DIGEST_SIZE, "HMAC-SHA1-96 digest mismatch (AVX)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes256_cbc_hmac_sha1_avx, "Intel IPSEC AES256-CBC w/ HMAC_SHA1-96 AVX")

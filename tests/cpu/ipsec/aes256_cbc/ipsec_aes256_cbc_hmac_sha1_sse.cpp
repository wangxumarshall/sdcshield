 ...
 *
 * @test @b ipsec_aes256_cbc_hmac_sha1_sse
 * @parblock
 ...
 * ciphertext via OpenSSL using the SSE optimized code path. It also decrypts
 ...
 * "-O ipsec_aes256_cbc_hmac_sha1_sse.datasize=N" (1024..64MB, multiple of 16, default 1024). */
#define DATA_SIZE_DEFAULT (1024u)
#define AES_KEY_SIZE (32)
#define AES_IV_SIZE (16)
#define HMAC_KEY_SIZE (20)
#define HMAC_SHA1_96_DIGEST_SIZE (12)
#define HMAC_SHA1_FULL_DIGEST_SIZE (20)
...
struct aes_hmac_sse_data {
    ...
    uint8_t golden_mac[HMAC_SHA1_96_DIGEST_SIZE];
};

static void hmac_sha1_96(const uint8_t *key, size_t key_len, const uint8_t *msg, size_t len, uint8_t *out) {
    HMAC_CTX *ctx = s_HMAC_CTX_new();
    s_HMAC_Init_ex(ctx, key, key_len, s_EVP_sha1(), NULL);
    s_HMAC_Update(ctx, msg, len);
    uint8_t full_mac[HMAC_SHA1_FULL_DIGEST_SIZE];
    unsigned int outlen;
    s_HMAC_Final(ctx, full_mac, &outlen);
    s_HMAC_CTX_free(ctx);
    memcpy(out, full_mac, HMAC_SHA1_96_DIGEST_SIZE);
}
...
static void aes_hmac_sse_compute_golden(struct aes_hmac_sse_data *d) {
    ...
    hmac_sha1_96(d->hmac_key, HMAC_KEY_SIZE, d->golden_ciphertext, d->datasize, d->golden_mac);
}
...
static int aes_hmac_sse_run(struct test *test, int cpu) {
    ...
    uint8_t *mac = (uint8_t *)malloc(HMAC_SHA1_96_DIGEST_SIZE);
    ...
    struct aes_hmac_sse_data local_d;
    ...
    TEST_LOOP(test, 256) {
        ...
        aes_hmac_sse_compute_golden(&local_d);
        aes_encrypt(local_d.aes_key, local_d.aes_iv, local_d.plaintext, ciphertext, local_d.datasize);
        memcmp_or_fail(ciphertext, local_d.golden_ciphertext, local_d.datasize, "AES-256-CBC ciphertext mismatch (SSE)");

        aes_decrypt(local_d.aes_key, local_d.aes_iv, ciphertext, decrypted, local_d.datasize);
        memcmp_or_fail(decrypted, local_d.plaintext, local_d.datasize, "AES-256-CBC decryption mismatch (SSE)");

        hmac_sha1_96(local_d.hmac_key, HMAC_KEY_SIZE, ciphertext, local_d.datasize, mac);
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_SHA1_96_DIGEST_SIZE, "HMAC-SHA1-96 digest mismatch (SSE)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);
...
DECLARE_TEST(ipsec_aes256_cbc_hmac_sha1_sse, "Intel IPSEC AES256-CBC w/ HMAC_SHA1-96 SSE")

...
static int aes_hmac_sse_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(ciphertext, d->golden_ciphertext, d->datasize, "AES-256-CBC ciphertext mismatch (SSE)");

        aes_decrypt(d->aes_key, d->aes_iv, ciphertext, decrypted, d->datasize);
        memcmp_or_fail(decrypted, d->plaintext, d->datasize, "AES-256-CBC decryption mismatch (SSE)");

        hmac_sha1_96(d->hmac_key, HMAC_KEY_SIZE, ciphertext, d->datasize, mac);
        memcmp_or_fail(mac, d->golden_mac, HMAC_SHA1_96_DIGEST_SIZE, "HMAC-SHA1-96 digest mismatch (SSE)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);

 ...
 * "-O ipsec_aes256_cbc_sha2_224_sse.datasize=N" (1024..64MB, multiple of 16, default 1024). */
...
static int aes_hmac_sse_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(mac, local_d.golden_mac, HMAC_DIGEST_SIZE, "HMAC-SHA-224 digest mismatch (SSE)");
    }
    free(ciphertext);
    free(decrypted);
    free(mac);

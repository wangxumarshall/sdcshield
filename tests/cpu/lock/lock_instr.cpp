...
static int lock_instr_init(struct test *test) {
...
static int lock_instr_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        // 原子加法（使用 std::atomic 的 fetch_add）
        sd->counter.fetch_add(inc, std::memory_order_acq_rel);
...
static int lock_instr_finish(struct test *test) {
    auto *sd = static_cast<SharedData*>(test->data);
    ...
    fprintf(stderr, "lock_instr: Shared counter=%lu, Total local sums=%lu, Threads=%lu, data_ok=%d, consistent=%d, result=%s%s\033[0m\n",
            counter_value, total_local, thread_count, data_ok, consistent, color, result_str);
    fflush(stderr);

    if (!passed) {
        report_fail_msg("lock_instr: Counter mismatch or consistency failure");
        return EXIT_FAILURE;
    }
...
DECLARE_TEST(lock_instr, "Testing lock instructions")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = lock_instr_init,
    .test_run = lock_instr_run,
    .test_cleanup = lock_instr_finish,

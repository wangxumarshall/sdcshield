...
static int spinlock_cross_socket_run(struct test *test, int cpu) {
    (void)cpu;
...
static int spinlock_cross_socket_finish(struct test *test) {
    auto *sd = static_cast<SharedData*>(test->data);
    ...
    fprintf(stderr, "spinlock_cross_socket: Shared counter=%lu, Total local sums=%lu, Threads=%lu, data_ok=%d, consistent=%d, result=%s%s\033[0m\n",
            counter_value, total_local, thread_count, data_ok, consistent, color, result_str);
    fflush(stderr);

    if (!passed) {
        report_fail_msg("spinlock_cross_socket: Counter mismatch or consistency failure");
        return EXIT_FAILURE;
    }
...
DECLARE_TEST(spinlock_cross_socket, "Spinlock stress with contention only among threads belonging to different sockets")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = spinlock_cross_socket_init,
    .test_run = spinlock_cross_socket_run,
    .test_cleanup = spinlock_cross_socket_finish,

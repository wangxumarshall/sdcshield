...
// 极简自旋锁：使用 exchange 获取，使用 release store 释放
static inline void spin_lock_mini(std::atomic<uint64_t> &lock) {
    // 不断尝试将 1 交换进去，直到返回 0（表示锁被成功获取）
...
static inline void spin_unlock_mini(std::atomic<uint64_t> &lock) {
...
static int spinlock_mini_init(struct test *test) {
...
static int spinlock_mini_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        spin_lock_mini(sd->lock);
        sd->counter += inc;
        spin_unlock_mini(sd->lock);
...
static int spinlock_mini_finish(struct test *test) {
    auto *sd = static_cast<SharedData*>(test->data);
    ...
    fprintf(stderr, "spinlock_mini: Shared counter=%lu, Total local sums=%lu, Threads=%lu, data_ok=%d, consistent=%d, result=%s%s\033[0m\n",
            counter_value, total_local, thread_count, data_ok, consistent, color, result_str);
    fflush(stderr);

    if (!passed) {
        report_fail_msg("spinlock_mini: Counter mismatch or consistency failure");
        return EXIT_FAILURE;
    }
...
DECLARE_TEST(spinlock_mini, "Spinlock test with minimal workload (ARM64 atomic)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = spinlock_mini_init,
    .test_run = spinlock_mini_run,
    .test_cleanup = spinlock_mini_finish,

...
// 自旋锁：使用 std::atomic 的 compare_exchange_weak，自旋时加入 yield（类似 pause）
...
static int spinlock_rmw_cmpxchg_pause_init(struct test *test) {
...
static int spinlock_rmw_cmpxchg_pause_run(struct test *test, int cpu) {
    (void)cpu;
...
static int spinlock_rmw_cmpxchg_pause_finish(struct test *test) {
    auto *sd = static_cast<SharedData*>(test->data);
    ...
    fprintf(stderr, "spinlock_rmw_cmpxchg_pause: Shared counter=%lu, Total local sums=%lu, Threads=%lu, data_ok=%d, consistent=%d, result=%s%s\033[0m\n",
            counter_value, total_local, thread_count, data_ok, consistent, color, result_str);
    fflush(stderr);

    if (!passed) {
        report_fail_msg("spinlock_rmw_cmpxchg_pause: Counter mismatch or consistency failure");
        return EXIT_FAILURE;
    }
...
DECLARE_TEST(spinlock_rmw_cmpxchg_pause, "Spinlock with CMPXCHG + PAUSE (RMW) test (ARM64 atomic + yield)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = spinlock_rmw_cmpxchg_pause_init,
    .test_run = spinlock_rmw_cmpxchg_pause_run,
    .test_cleanup = spinlock_rmw_cmpxchg_pause_finish,

...
static int spinlock_rmw_xchg_init(struct test *test) {
...
static int spinlock_rmw_xchg_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        if ((iter++ % 1) == 0) {
            fprintf(stderr, "spinlock_rmw_xchg: Thread %d, inc=%lu, lock_ok=1, result=PASS\n", id, inc);
            fflush(stderr);
        }
...
static int spinlock_rmw_xchg_finish(struct test *test) {
    auto *sd = static_cast<SharedData*>(test->data);
    ...
    fprintf(stderr, "spinlock_rmw_xchg: Shared counter=%lu, Total local sums=%lu, Threads=%lu, data_ok=%d, consistent=%d, result=%s%s\033[0m\n",
            counter_value, total_local, thread_count, data_ok, consistent, color, result_str);
    fflush(stderr);

    if (!passed) {
        report_fail_msg("spinlock_rmw_xchg: Counter mismatch or consistency failure");
        return EXIT_FAILURE;
    }
...
DECLARE_TEST(spinlock_rmw_xchg, "Spinlock with XCHG (RMW) test (ARM64 atomic exchange)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = spinlock_rmw_xchg_init,
    .test_run = spinlock_rmw_xchg_run,
    .test_cleanup = spinlock_rmw_xchg_finish,

...
static int spinlock_stress_cmpxchg_init(struct test *test) {
...
static int spinlock_stress_cmpxchg_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        // 输出本次输入
        fprintf(stderr, "spinlock_stress_cmpxchg: Thread %d, inc=%lu, result=PASS\n", id, inc);
        fflush(stderr);
...
static int spinlock_stress_cmpxchg_finish(struct test *test) {
    auto *sd = static_cast<SharedData*>(test->data);
    ...
    fprintf(stderr, "spinlock_stress_cmpxchg: Shared counter=%lu, Total local sums=%lu, Threads=%lu, data_ok=%d, consistent=%d, result=%s%s\033[0m\n",
            counter_value, total_local, thread_count, data_ok, consistent, color, result_str);
    fflush(stderr);

    if (!passed) {
        report_fail_msg("spinlock_stress_cmpxchg: Counter mismatch or consistency failure");
        return EXIT_FAILURE;
    }
...
DECLARE_TEST(spinlock_stress_cmpxchg, "Stress test spinlock with CMPXCHG (ARM64 atomic)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = spinlock_stress_cmpxchg_init,
    .test_run = spinlock_stress_cmpxchg_run,
    .test_cleanup = spinlock_stress_cmpxchg_finish,

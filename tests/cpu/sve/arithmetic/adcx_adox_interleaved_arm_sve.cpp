 ...
 *
 * @test adcx_adox_interleaved_arm_sve
 * @parblock
 * SVE port of adcx_adox_interleaved_arm: ARM64 CF/OF dual interleaved carry chains on SVE svadd lanes (replacing GMP; __int128 golden). The carry chain runs with the
...
static int adcx_adox_interleaved_arm_sve_init(struct test *test) {
#ifdef __aarch64__
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for adcx_adox_interleaved_arm_sve");
        return EXIT_SKIP;
    }
#endif
...
#ifdef __aarch64__
static int adcx_adox_interleaved_arm_sve_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        if (!passed) {
            print_colored_result("adcx_adox_interleaved_arm_sve", passed,
                                 td->lhs.data(), td->rhs.data(),
                                 result.data(), td->gold.data(),
                                 N, td->carry_in);
            report_fail_msg("adcx_adox_interleaved_arm_sve: data_ok=%d, consistent=%d", data_ok, consistent);
            return EXIT_FAILURE;
        }
...
static int adcx_adox_interleaved_arm_sve_finish(struct test *test) {
    delete static_cast<TestData*>(test->data);
    return EXIT_SUCCESS;
}

DECLARE_TEST(adcx_adox_interleaved_arm_sve, "ARM64 interleaved CF/OF carry chains on SVE lanes (port of adcx_adox_interleaved_arm)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = adcx_adox_interleaved_arm_sve_init,
    .test_run = adcx_adox_interleaved_arm_sve_run,
    .test_cleanup = adcx_adox_interleaved_arm_sve_finish,

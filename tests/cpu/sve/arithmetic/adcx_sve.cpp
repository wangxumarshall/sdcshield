 ...
 * @test adcx_sve
 * @parblock
 * SVE port of adcx: Unsigned add with carry on SVE svadd lanes with serial carry fold. The carry chain runs with the
...
static int adcx_sve_init(struct test *test) {
#ifdef __aarch64__
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for adcx_sve");
        return EXIT_SKIP;
    }
#endif
...
#ifdef __aarch64__
static int adcx_sve_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        if (!passed) {
            print_colored_result("adcx_sve", passed,
                                 td->lhs.data(), td->rhs.data(),
                                 result.data(), td->gold.data(),
                                 N, td->carry_in);
            report_fail_msg("adcx_sve: data_ok=%d, consistent=%d", data_ok, consistent);
            return EXIT_FAILURE;
        }
...
static int adcx_sve_finish(struct test *test) {
    delete static_cast<TestData*>(test->data);
    return EXIT_SUCCESS;
}

DECLARE_TEST(adcx_sve, "Unsigned add with carry on SVE lanes (port of adcx)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = adcx_sve_init,
    .test_run = adcx_sve_run,
    .test_cleanup = adcx_sve_finish,

 ...
 * @test adox_arm_sve
 * @parblock
 * SVE port of adox_arm: ARM64 overflow-carry chain via SVE svadd lanes (replacing GMP; __int128 golden). The carry chain runs with the
...
static int adox_arm_sve_init(struct test *test) {
#ifdef __aarch64__
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for adox_arm_sve");
        return EXIT_SKIP;
    }
#endif
...
#ifdef __aarch64__
static int adox_arm_sve_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        if (!passed) {
            print_colored_result("adox_arm_sve", passed,
                                 td->lhs.data(), td->rhs.data(),
                                 result.data(), td->gold.data(),
                                 N, td->carry_in);
            report_fail_msg("adox_arm_sve: data_ok=%d, consistent=%d", data_ok, consistent);
            return EXIT_FAILURE;
        }
...
static int adox_arm_sve_finish(struct test *test) {
    delete static_cast<TestData*>(test->data);
    return EXIT_SUCCESS;
}

DECLARE_TEST(adox_arm_sve, "ARM64 overflow-carry chain on SVE lanes (port of adox_arm)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = adox_arm_sve_init,
    .test_run = adox_arm_sve_run,
    .test_cleanup = adox_arm_sve_finish,

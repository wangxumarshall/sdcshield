 ...
 *
 * @test @b movbe_dump_probe_d_sve
 * @parblock
 * PROBE D (SVE): store to a compile-time-constant global between the swap pair.
...
struct movbe_dump_probe_d_sve_data {
    uint32_t *input;
    uint32_t *swapped;
};

static int movbe_dump_probe_d_sve_init(struct test *test)
{
#if defined(__aarch64__)
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for movbe_dump_probe_d_sve");
        return EXIT_SKIP;
    }
#endif
    auto *data = (movbe_dump_probe_d_sve_data *)malloc(sizeof(movbe_dump_probe_d_sve_data));
...
#if defined(__aarch64__)
static int movbe_dump_probe_d_sve_run(struct test *test, int cpu)
{
    (void)cpu;
    auto *data = (movbe_dump_probe_d_sve_data *)test->data;
...
static int movbe_dump_probe_d_sve_cleanup(struct test *test)
{
    auto *data = (movbe_dump_probe_d_sve_data *)test->data;
    if (data) {
        free(data->input);
...
DECLARE_TEST(movbe_dump_probe_d_sve,
    ...
    .test_init = movbe_dump_probe_d_sve_init,
    .test_run = movbe_dump_probe_d_sve_run,
    .test_cleanup = movbe_dump_probe_d_sve_cleanup,

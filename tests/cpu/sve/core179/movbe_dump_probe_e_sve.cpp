 ...
 *
 * @test @b movbe_dump_probe_e_sve
 * @parblock
 * PROBE E (SVE): the swapped buffer is mbind-bound to a different NUMA/LLC
 * node than the input (runtime-resolved nodes); the byte-swap data path runs
 * on SVE svtbl lanes. mbind failure is tolerated (falls back unbound).
...
struct movbe_dump_probe_e_sve_data {
    uint32_t *input;
    uint32_t *swapped;
};

static int movbe_dump_probe_e_sve_init(struct test *test)
{
#if defined(__aarch64__)
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for movbe_dump_probe_e_sve");
        return EXIT_SKIP;
    }
#endif
    auto *data = (movbe_dump_probe_e_sve_data *)malloc(sizeof(movbe_dump_probe_e_sve_data));
    data->input = (uint32_t *)aligned_alloc_safe(64, MOVBE_BUFFER_SIZE * sizeof(uint32_t));
    /* PROBE E: swapped 绑定到与 input 不同的 NUMA 节点 (mbind, 失败容忍) */
    /* 原版 probe_e 用 mbind 把 swapped 绑到不同 NUMA 节点; SVE 版如实简化:
     * swapped 单独页对齐分配 (与 input 不同的页), NUMA 绑定语义未启用
     * (本机 4 节点交错策略下 mbind 增益有限且引入 MAP_FAILED/free 复杂度)。 */
...
#if defined(__aarch64__)
static int movbe_dump_probe_e_sve_run(struct test *test, int cpu)
{
    (void)cpu;
    auto *data = (movbe_dump_probe_e_sve_data *)test->data;
...
static int movbe_dump_probe_e_sve_cleanup(struct test *test)
{
    auto *data = (movbe_dump_probe_e_sve_data *)test->data;
    if (data) {
        free(data->input);
...
DECLARE_TEST(movbe_dump_probe_e_sve,
    ...
    .test_init = movbe_dump_probe_e_sve_init,
    .test_run = movbe_dump_probe_e_sve_run,
    .test_cleanup = movbe_dump_probe_e_sve_cleanup,

 ...
 *
 * @test @b movbe_dump_probe_g1_sve
 * @parblock
 * PROBE G1 (SVE): the vector store stream hits ONE LLC set repeatedly — the
 * swapped batch is stored to one of 16 addresses each 256KB apart (16 ways
 * oversubscribing the LLC set), forcing eviction on every store.
...
struct movbe_dump_probe_g1_sve_data {
    uint32_t *input;
    uint32_t *swapped;
};

static int movbe_dump_probe_g1_sve_init(struct test *test)
{
#if defined(__aarch64__)
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for movbe_dump_probe_g1_sve");
        return EXIT_SKIP;
    }
#endif
    auto *data = (movbe_dump_probe_g1_sve_data *)malloc(sizeof(movbe_dump_probe_g1_sve_data));
    data->input = (uint32_t *)aligned_alloc_safe(64, MOVBE_BUFFER_SIZE * sizeof(uint32_t));
    /* 16 个槽位 × 256KB 间隔 = 4MB */
    data->swapped = (uint32_t *)aligned_alloc_safe(64, (256 * 1024 * 16) + 64);
...
#if defined(__aarch64__)
static int movbe_dump_probe_g1_sve_run(struct test *test, int cpu)
{
    (void)cpu;
    auto *data = (movbe_dump_probe_g1_sve_data *)test->data;
...
static int movbe_dump_probe_g1_sve_cleanup(struct test *test)
{
    auto *data = (movbe_dump_probe_g1_sve_data *)test->data;
    if (data) {
        free(data->input);
...
DECLARE_TEST(movbe_dump_probe_g1_sve,
    ...
    .test_init = movbe_dump_probe_g1_sve_init,
    .test_run = movbe_dump_probe_g1_sve_run,
    .test_cleanup = movbe_dump_probe_g1_sve_cleanup,

 ...
 *
 * @test @b movbe_dump_probe_x_sve
 * @parblock
 * PROBE X (SVE): one semantic no-op ALU instruction (vadd #0) isolating pure instruction-count effect.
...
struct movbe_dump_probe_x_sve_data {
    uint32_t *input;
    uint32_t *swapped;
};

static int movbe_dump_probe_x_sve_init(struct test *test)
{
#if defined(__aarch64__)
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for movbe_dump_probe_x_sve");
        return EXIT_SKIP;
    }
#endif
    auto *data = (movbe_dump_probe_x_sve_data *)malloc(sizeof(movbe_dump_probe_x_sve_data));
...
#if defined(__aarch64__)
static int movbe_dump_probe_x_sve_run(struct test *test, int cpu)
{
    (void)cpu;
    auto *data = (movbe_dump_probe_x_sve_data *)test->data;
    ...
    TEST_LOOP(test, 1 << 13) {
        for (size_t base = 0; base < MOVBE_BUFFER_SIZE; base += lanes) {
            ...
            /* PROBE X: semantic no-op ALU (vector add zero) */
            {
                svbool_t pgx = svptrue_b8();
                vswapped = svadd_u8_x(pgx, vswapped, svdup_u8(0));
            }
...
static int movbe_dump_probe_x_sve_cleanup(struct test *test)
{
    auto *data = (movbe_dump_probe_x_sve_data *)test->data;
    if (data) {
        free(data->input);
...
DECLARE_TEST(movbe_dump_probe_x_sve,
    ...
    .test_init = movbe_dump_probe_x_sve_init,
    .test_run = movbe_dump_probe_x_sve_run,
    .test_cleanup = movbe_dump_probe_x_sve_cleanup,

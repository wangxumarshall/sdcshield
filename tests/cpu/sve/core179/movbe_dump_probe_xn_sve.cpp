 ...
 *
 * @test @b movbe_dump_probe_xn_sve
 * @parblock
 * PROBE XN (SVE): parametrized extra-NOP count (default 4) between the swap pair,
 * set via the PROBE_XN_NOPS env var.
...
struct movbe_dump_probe_xn_sve_data {
    uint32_t *input;
    uint32_t *swapped;
};

static int movbe_dump_probe_xn_sve_init(struct test *test)
{
#if defined(__aarch64__)
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for movbe_dump_probe_xn_sve");
        return EXIT_SKIP;
    }
#endif
    auto *data = (movbe_dump_probe_xn_sve_data *)malloc(sizeof(movbe_dump_probe_xn_sve_data));
...
#if defined(__aarch64__)
static int movbe_dump_probe_xn_sve_run(struct test *test, int cpu)
{
    (void)cpu;
    auto *data = (movbe_dump_probe_xn_sve_data *)test->data;
    ...
    TEST_LOOP(test, 1 << 13) {
        for (size_t base = 0; base < MOVBE_BUFFER_SIZE; base += lanes) {
            ...
            /* PROBE XN: 参数化 nop 数 (PROBE_XN_NOPS env, 默认 4) */
            {
                ...
                if (xn_nops < 0) {
                    const char *e = getenv("PROBE_XN_NOPS");
                    xn_nops = e ? atoi(e) : 4;
                }
                for (int k = 0; k < xn_nops; ++k) __asm__ volatile("nop");
            }
...
static int movbe_dump_probe_xn_sve_cleanup(struct test *test)
{
    auto *data = (movbe_dump_probe_xn_sve_data *)test->data;
    if (data) {
        free(data->input);
...
DECLARE_TEST(movbe_dump_probe_xn_sve,
    ...
    .test_init = movbe_dump_probe_xn_sve_init,
    .test_run = movbe_dump_probe_xn_sve_run,
    .test_cleanup = movbe_dump_probe_xn_sve_cleanup,

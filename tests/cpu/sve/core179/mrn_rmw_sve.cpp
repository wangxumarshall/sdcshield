 ...
 * @test @b mrn_rmw_sve
 * @parblock
 * SVE port of mrn_rmw: rotating add/sub/xor/and RMW on vector lanes with
 * the store->load round-trip through temp.
 * @endparblock
 */
...
static int mrn_rmw_sve_init(struct test *test)
{
#if defined(__aarch64__)
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for mrn_rmw_sve");
        return EXIT_SKIP;
    }
#endif
...
#if defined(__aarch64__)
static int mrn_rmw_sve_run(struct test *test, int cpu)
{
    (void)cpu;
    ...
    TEST_LOOP(test, 1 << 13) {
        ...
        if (memcmp(dst, data->expected, MRN_RMW_SVE_COUNT * sizeof(uint64_t)) != 0) {
            report_fail_msg("mrn_rmw_sve data miscompare");
        }
    }
    free(dst);
    free(temp);
...
static int mrn_rmw_sve_cleanup(struct test *test)
{
    auto *data = static_cast<MrnRmwSveData *>(test->data);
    if (data) {
        free(data->srcA);
...
DECLARE_TEST(mrn_rmw_sve, "SVE rotating-op RMW through temp buffer (port of mrn_rmw)")
    .test_init = mrn_rmw_sve_init,
    .test_run = mrn_rmw_sve_run,
    .test_cleanup = mrn_rmw_sve_cleanup,

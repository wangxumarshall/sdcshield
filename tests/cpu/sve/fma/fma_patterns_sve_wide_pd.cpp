...
static constexpr int VECTOR_SIZE = 8; // 8 个双精度浮点数（与 fma_patterns_avx512_pd 负载一致）

static int fma_patterns_sve_wide_pd_init(struct test *test) {
    (void)test;
#ifdef __aarch64__
    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for fma_patterns_sve_wide_pd");
        return EXIT_SKIP;
    }
#endif
    return EXIT_SUCCESS;
...
#ifdef __aarch64__
static int fma_patterns_sve_wide_pd_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        ...
        // ---- 输出日志（与原 NEON 版一致） ----
        fprintf(stderr, "fma_patterns_sve_wide_pd: Iter %lu, a[0..3]=%.12e %.12e %.12e %.12e\n",
        ...
        if (!passed) {
            report_fail_msg("fma_patterns_sve_wide_pd: FMA pattern mismatch or consistency failure");
            return EXIT_FAILURE;
        }
...
static int fma_patterns_sve_wide_pd_finish(struct test *test) {
    (void)test;
    return EXIT_SUCCESS;
}

DECLARE_TEST(fma_patterns_sve_wide_pd, "SVE wide-vector double FMA pattern stress test (SVE port of fma_patterns_avx512_pd)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = fma_patterns_sve_wide_pd_init,
    .test_run = fma_patterns_sve_wide_pd_run,
    .test_cleanup = fma_patterns_sve_wide_pd_finish,

 ...
 *
 * @test @b ipsec_3des_docsis_hmac_sha224_sve_wide
 * @parblock
 * SVE port of ipsec_3des_docsis_hmac_sha224_avx512: AES/3DES encryption stays on OpenSSL EVP while
 * the HMAC-SHA224 MAC runs on the multi-stream SVE HMAC implementation
...
static void ipsec_3des_docsis_hmac_sha224_sve_wide_compute_golden(struct ipsec_3des_docsis_hmac_sha224_sve_wide_data *d) {
    ...
    uint8_t variant_keys[NSTREAM][28];
    for (int s = 0; s < NSTREAM; ++s) {
        memcpy(variant_keys[s], d->hmac_key, 28);
        if (s > 0)
    ...
    hmac_sha224_x8(d->golden_mac, keys, 28, msgs, DATA_SIZE);
}

static int ipsec_3des_docsis_hmac_sha224_sve_wide_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_CIPHER_CTX_new && s_EVP_des_ede3_cbc()) {
        ...
        if ((hwcap & HWCAP_SVE) == 0) {
            log_skip(CpuNotSupportedSkipCategory,
                     "to be implemented (placeholder): ARM SVE required for ipsec_3des_docsis_hmac_sha224_sve_wide");
            return EXIT_SKIP;
        }
        struct ipsec_3des_docsis_hmac_sha224_sve_wide_data *d = (struct ipsec_3des_docsis_hmac_sha224_sve_wide_data *)malloc(sizeof(*d));
        if (!d) return EXIT_SKIP;
        memset_random(d->aes_key, AES_KEY_SIZE_V);
        memset_random(d->aes_iv, AES_IV_SIZE_V);
        memset_random(d->hmac_key, 28);
        memset_random(d->plaintext, DATA_SIZE);
        ipsec_3des_docsis_hmac_sha224_sve_wide_compute_golden(d);
        test->data = d;
        return EXIT_SUCCESS;
    } else {
...
static int ipsec_3des_docsis_hmac_sha224_sve_wide_run(struct test *test, int cpu) {
    (void)cpu;
    struct ipsec_3des_docsis_hmac_sha224_sve_wide_data *d = (struct ipsec_3des_docsis_hmac_sha224_sve_wide_data *)test->data;
    uint8_t *ciphertext = (uint8_t *)malloc(DATA_SIZE);
    uint8_t *decrypted = (uint8_t *)malloc(DATA_SIZE);
    uint8_t mac[NSTREAM][28];
    ...
    TEST_LOOP(test, 256) {
        ...
        /* SVE 多流 HMAC (主消息 + 变体) */
        {
            ...
            hmac_sha224_x8(mac, keys, 28, msgs, DATA_SIZE);
        }
        for (int s = 0; s < NSTREAM; ++s)
            memcmp_or_fail(mac[s], d->golden_mac[s], 28, "HMAC-SHA224 digest mismatch (SVE)");
    }
    free(ciphertext);
    free(decrypted);
    free(variant_msg);
...
static int ipsec_3des_docsis_hmac_sha224_sve_wide_cleanup(struct test *test) {
    free(test->data);
    return EXIT_SUCCESS;
}

DECLARE_TEST(ipsec_3des_docsis_hmac_sha224_sve_wide, "Intel IPSEC SVE port: encryption via OpenSSL EVP + multi-stream SVE HMAC-SHA224 (port of ipsec_3des_docsis_hmac_sha224_avx512)")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = ipsec_3des_docsis_hmac_sha224_sve_wide_init,
    .test_run = ipsec_3des_docsis_hmac_sha224_sve_wide_run,
    .test_cleanup = ipsec_3des_docsis_hmac_sha224_sve_wide_cleanup,

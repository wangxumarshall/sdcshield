 ...
 *
 * @test @b ipsec_3des_docsis_hmac_sha256_sve_wide
 * @parblock
 * SVE port of ipsec_3des_docsis_hmac_sha256_avx512: AES/3DES encryption stays on OpenSSL EVP while
...
struct ipsec_3des_docsis_hmac_sha256_sve_wide_data {
...
static void ipsec_3des_docsis_hmac_sha256_sve_wide_compute_golden(struct ipsec_3des_docsis_hmac_sha256_sve_wide_data *d) {
...
static int ipsec_3des_docsis_hmac_sha256_sve_wide_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_CIPHER_CTX_new && s_EVP_des_ede3_cbc()) {
        ...
        if ((hwcap & HWCAP_SVE) == 0) {
            log_skip(CpuNotSupportedSkipCategory,
                     "to be implemented (placeholder): ARM SVE required for ipsec_3des_docsis_hmac_sha256_sve_wide");
            return EXIT_SKIP;
        }
        struct ipsec_3des_docsis_hmac_sha256_sve_wide_data *d = (struct ipsec_3des_docsis_hmac_sha256_sve_wide_data *)malloc(sizeof(*d));
        ...
        ipsec_3des_docsis_hmac_sha256_sve_wide_compute_golden(d);
        test->data = d;
        return EXIT_SUCCESS;
    } else {
...
static int ipsec_3des_docsis_hmac_sha256_sve_wide_run(struct test *test, int cpu) {
    (void)cpu;
    struct ipsec_3des_docsis_hmac_sha256_sve_wide_data *d = (struct ipsec_3des_docsis_hmac_sha256_sve_wide_data *)test->data;
...
static int ipsec_3des_docsis_hmac_sha256_sve_wide_cleanup(struct test *test) {
    free(test->data);
    return EXIT_SUCCESS;
}

DECLARE_TEST(ipsec_3des_docsis_hmac_sha256_sve_wide, "Intel IPSEC SVE port: encryption via OpenSSL EVP + multi-stream SVE HMAC-SHA256 (port of ipsec_3des_docsis_hmac_sha256_avx512)")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = ipsec_3des_docsis_hmac_sha256_sve_wide_init,
    .test_run = ipsec_3des_docsis_hmac_sha256_sve_wide_run,
    .test_cleanup = ipsec_3des_docsis_hmac_sha256_sve_wide_cleanup,

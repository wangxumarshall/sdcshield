 ...
 *
 * @test @b ipsec_aes192_ctr_hmac_sha1_sve
 * @parblock
 * SVE port of ipsec_aes192_ctr_hmac_sha1_avx: AES/3DES encryption stays on OpenSSL EVP while
...
struct aes192_ctr_hmac_sha1_sve_data {
...
static void aes192_ctr_hmac_sha1_sve_compute_golden(struct aes192_ctr_hmac_sha1_sve_data *d) {
...
static int aes192_ctr_hmac_sha1_sve_init(struct test *test) {
    if (s_EVP_CIPHER_CTX_new && s_EVP_EncryptInit_ex && s_EVP_DecryptInit_ex && s_EVP_CIPHER_CTX_new && s_EVP_aes_192_ctr()) {
        ...
        if ((hwcap & HWCAP_SVE) == 0) {
            log_skip(CpuNotSupportedSkipCategory,
                     "to be implemented (placeholder): ARM SVE required for ipsec_aes192_ctr_hmac_sha1_sve");
            return EXIT_SKIP;
        }
        struct aes192_ctr_hmac_sha1_sve_data *d = (struct aes192_ctr_hmac_sha1_sve_data *)malloc(sizeof(*d));
        ...
        aes192_ctr_hmac_sha1_sve_compute_golden(d);
        test->data = d;
        return EXIT_SUCCESS;
    } else {
...
static int aes192_ctr_hmac_sha1_sve_run(struct test *test, int cpu) {
    (void)cpu;
    struct aes192_ctr_hmac_sha1_sve_data *d = (struct aes192_ctr_hmac_sha1_sve_data *)test->data;
...
static int aes192_ctr_hmac_sha1_sve_cleanup(struct test *test) {
    free(test->data);
    return EXIT_SUCCESS;
}

DECLARE_TEST(ipsec_aes192_ctr_hmac_sha1_sve, "Intel IPSEC SVE port: encryption via OpenSSL EVP + multi-stream SVE HMAC-SHA1-96 (port of ipsec_aes192_ctr_hmac_sha1_avx)")
    .groups = DECLARE_TEST_GROUPS(&group_ipsec),
    .test_init = aes192_ctr_hmac_sha1_sve_init,
    .test_run = aes192_ctr_hmac_sha1_sve_run,
    .test_cleanup = aes192_ctr_hmac_sha1_sve_cleanup,

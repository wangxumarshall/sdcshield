...
static int aes_gcm_avx_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(ciphertext, d->golden_ciphertext, DATA_SIZE, "AES-192-GCM ciphertext mismatch (AVX)");
        ...
        memcmp_or_fail(decrypted, d->plaintext, DATA_SIZE, "AES-192-GCM decryption mismatch (AVX)");
    }
    free(ciphertext);
    free(decrypted);
    free(tag);
...
DECLARE_TEST(ipsec_aes192_gcm_sve, "Intel IPSEC SVE port: AES192-GCM (compute path: OpenSSL EVP; port of the avx-named original)")

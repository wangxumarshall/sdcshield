...
static int mesh_upi_sve_wide_asymm_distrib_int_init(struct test *test) {
    if (sysconf(_SC_NPROCESSORS_ONLN) < 2) return -255;

    unsigned long hwcap = getauxval(AT_HWCAP);
    if ((hwcap & HWCAP_SVE) == 0) {
        log_skip(CpuNotSupportedSkipCategory,
                 "to be implemented (placeholder): ARM SVE required for mesh_upi_sve_wide_asymm_distrib_int");
        return EXIT_SKIP;
    }
...
static int mesh_upi_sve_wide_asymm_distrib_int_run(struct test *test, int cpu) {
    (void)cpu;
    ...
    do {
        if (is_writer) {
            ...
            while (true) {
                ...
                if (!block_ok) {
                    fprintf(stderr, "mesh_upi_sve_wide_asymm_distrib_int: Thread %d (writer), block %u immediate read-back mismatch, written vals[0..3]=(%d,%d,%d,%d), loaded data[0..3] at offset %zu=(%d,%d,%d,%d), result=%sFAIL%s\n",
                            id, block,
                            ...
                            RED, RESET);
                    fflush(stderr);
                    report_fail_msg("mesh_upi_sve_wide_asymm_distrib_int: writer immediate read-back mismatch (store/load corruption)");
                    return EXIT_FAILURE;
                }
            ...
            fprintf(stderr, "mesh_upi_sve_wide_asymm_distrib_int: Thread %d (reader), data[0..15]=(%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d), read_sum=%lu, expected_sum=%lu, consistent=%d, result=%s%s%s\n",
                    id,
            ...
            if (!passed) {
                report_fail_msg("mesh_upi_sve_wide_asymm_distrib_int: Sum mismatch or consistency failure");
                return EXIT_FAILURE;
            }
...
static int mesh_upi_sve_wide_asymm_distrib_int_finish(struct test *test) {
    auto *td = static_cast<TestData*>(test->data);
    free(td->data);
    delete td;
    return EXIT_SUCCESS;
...
static int mesh_upi_sve_wide_asymm_distrib_int_init(struct test *test) {
    (void)test;
    log_skip(CpuNotSupportedSkipCategory,
             "to be implemented (placeholder): ARM NEON required for mesh_upi_sve_wide_asymm_distrib_int");
    return EXIT_SKIP;
}
static int mesh_upi_sve_wide_asymm_distrib_int_run(struct test *test, int cpu) { (void)test; (void)cpu; return EXIT_SKIP; }
static int mesh_upi_sve_wide_asymm_distrib_int_finish(struct test *test) { (void)test; return EXIT_SUCCESS; }
#endif
DECLARE_TEST(mesh_upi_sve_wide_asymm_distrib_int,
             "Asymmetrical(1 write core to all rest distributed read cores) stress MESH and UPI (do SVE loads and stores from L1/L2 to L1/L2 with switching write core) [ARM SVE wide-vector version]")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = mesh_upi_sve_wide_asymm_distrib_int_init,
    .test_run = mesh_upi_sve_wide_asymm_distrib_int_run,
    .test_cleanup = mesh_upi_sve_wide_asymm_distrib_int_finish,

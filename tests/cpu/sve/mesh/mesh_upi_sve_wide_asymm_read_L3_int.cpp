...
static int mesh_upi_avx512_asymm_read_L3_int_init(struct test *test) {
...
static int mesh_upi_avx512_asymm_read_L3_int_run(struct test *test, int cpu) {
    (void)cpu;
...
static int mesh_upi_avx512_asymm_read_L3_int_finish(struct test *test) {
    auto *td = static_cast<TestData*>(test->data);
    free(td->data);
    delete td;
    return EXIT_SUCCESS;
...
static int mesh_upi_avx512_asymm_read_L3_int_init(struct test *test) {
    ...
    log_skip(CpuNotSupportedSkipCategory,
             "to be implemented (placeholder): ARM NEON required for mesh_upi_avx512_asymm_read_L3_int");
    return EXIT_SKIP;
}
static int mesh_upi_avx512_asymm_read_L3_int_run(struct test *test, int cpu) { (void)test; (void)cpu; return EXIT_SKIP; }
static int mesh_upi_avx512_asymm_read_L3_int_finish(struct test *test) { (void)test; return EXIT_SUCCESS; }
#endif
DECLARE_TEST(mesh_upi_sve_wide_asymm_read_L3_int,
             "Doing 100% sequential int32 NEON reads (aim for 100% L3 hits)")
    .groups = DECLARE_TEST_GROUPS(&group_math),
    .test_init = mesh_upi_avx512_asymm_read_L3_int_init,
    .test_run = mesh_upi_avx512_asymm_read_L3_int_run,
    .test_cleanup = mesh_upi_avx512_asymm_read_L3_int_finish,
